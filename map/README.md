...
**Live at:** https://climaiq.tech/map
...
- Camp Fire region (Paradise, CA):
  https://climaiq.tech/map?bbox=-121.7,39.5,-121.2,40.0
- Mt. Whitney area:
  https://climaiq.tech/map?bbox=-118.5,36.4,-118.1,36.7
- Lake Tahoe basin:
  https://climaiq.tech/map?bbox=-120.2,38.9,-119.9,39.2
...
repo. Two paths to upgrade `/map` from demo to live:
...
issue). Then update the page's `fetch('/map/data/fires.json')` to
...
" > path/to/website/map/data/fires.json
```
...
git commit -m "feat(map): add /map historical fires explorer"
git push
```

Vercel auto-deploys. The `vercel.json` rewrite for `/map → /map/index.html`
makes `/map` work alongside the existing `/dashboard`, `/platform`, `/videos`
clean URLs.
